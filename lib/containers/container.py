...
class Container:
    ...
    def read_from_containers(self, path: str):
        ...
        x, err = self._container_index.load_from_index(CONTAINERS_STRING, path)
        if err:
            raise err
    ...
    def read_primitive_value(self, path: str):
        ...
        value = self.read_from_value(new_path)
...
def build_container_tree(current_container=None, path=None, path_delim=None, start=None):
    ...
    if current_container is None:
        ...
        current_container = new_container_func(None, ".", "", start)
        path = [""]
